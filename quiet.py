    ...
    def actuate_lights(self):
        ...
        while True:
            if self.conflict_groups:
                ...
                total_cycle_limit = min(
                    max_cycle_time, baseline_cycle_time + scaling_factor * avg_pressure
                )
                total_green_time = sum(
                    sum(light.green_time for light in group)
                    for group in self.conflict_groups
                )
                total_fixed_time = sum(
                    sum(
                        light.red_time + light.amber_time + light.red_amber_time
                        for light in group
                    )
                    for group in self.conflict_groups
                )
                if total_green_time + total_fixed_time > total_cycle_limit:
                    scale_factor = (
                        total_cycle_limit - total_fixed_time
                    ) / total_green_time
                    for group in self.conflict_groups:
                        for light in group:
...
class FRoad:
    def __init__(
        ...
        speed: int,
        distance: int,
        junction_start: FJunction,
        junction_end: FJunction,
    ...
    def run(self):
        ...
        while True:
            ...
            with self.road.junction_start.queue.request(priority=1) as request:
                yield request
                available_distance = self.road.distance - sum(
                    car.length for car in self.road.car_queue.items
                )
                while (
                    self.road.traffic_light.colour in ["RED", "RAMBER", "AMBER"]
                    or self.road.car_queue.items[0] != self
                    or (
                        self.road.distance
                        - sum(car.length for car in self.road.car_queue.items)
                    )
                    < (self.length + SAFETY_GAP)
                ):
                    available_distance = self.road.distance - sum(
                        car.length for car in self.road.car_queue.items
                    )
                    if available_distance < (self.length + SAFETY_GAP):
                        if red_cycle_count < 2:
                            ...
                            print(
                                f"[{self.env.now}] {self.name} reordering to front after {red_cycle_count} cycles."
                            )
                    ...
                    yield self.env.timeout(self.reaction_time)
                red_cycle_count = 0
                start_delay = sample_reaction_time(
                    mean=0.8, std=0.1, lower=0.5, upper=1.0
                )
            ...
            v_max = math.sqrt(
                self.speed**2 + 2 * self.acceleration * available_distance
            )
            ...
            v_p = math.sqrt(
                (
                    self.decceleration * self.speed**2
                    - self.acceleration * final_v**2
            ...
            possible_roads = [
                ...
                and road.junction_end
                != self.road.junction_start  # Exclude roads that loop back
                and (
                    sum(car.length for car in road.car_queue.items) + self.length
                    < road.distance
                )
            ]
            weights = []
