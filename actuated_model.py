...
from fixed import (
    FJunctionFixed,
)  # We use the same junction class for interface convenience.
from actuated import (
    ATrafficLightActuated,
)  # Adaptive/actuated traffic light implementation.
...
from config import (
    ...
    BASE_MEAN,
    RANDOM_SEED,
    ACTUATED_TIMINGS_CSV,
    DEFAULT_ACT_RED_TIME,
    DEFAULT_ACT_GREEN_TIME,
    DEFAULT_ACT_AMBER_TIME,
    DEFAULT_ACT_RED_AMBER_TIME,
...
def create_junction(env, i, j, total_rows, total_cols):
    ...
    if (
        (i == 0 and j == 0)
        or (i == 0 and j == total_cols - 1)
        or (i == total_rows - 1 and j == 0)
        or (i == total_rows - 1 and j == total_cols - 1)
    ):
...
def actuated_main(
    ...
    startable = [
        road
        for road in roads
        if hasattr(road.junction_start, "start")
        and road.junction_start.start
        and not getattr(road.junction_end, "end", False)
    ]
    print(f"Found {len(startable)} startable roads with strict filter.")
    if not startable:
        print(
            "No startable roads found using strict filter; falling back to all roads."
        )
        startable = roads
