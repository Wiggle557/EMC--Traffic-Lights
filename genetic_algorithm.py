...
from config import (GRID_ROWS, GRID_COLS, RANDOM_SEED,
                    GA_GENERATIONS, GA_POPULATION_SIZE, GA_MUTATION_RATE,
                    GA_MUTATION_STRENGTH, GA_THRESHOLD, GA_PENALTY_FACTOR)
...
def construct_candidate_timings(candidate, roads):
    ...
    for road in roads:
        ...
        group_key = road.junction_start.name if "Junction_" in road.junction_start.name else road.junction_start.name
...
def evaluate_candidate(candidate):
    ...
    if _global_roads is None:
        sim_result = fixed_main(headless=True, rows=GRID_ROWS, cols=GRID_COLS, sim_duration=10,
                                random_seed=random.randint(1, 100000))
        _global_roads = sim_result["roads"]
    candidate_timings = construct_candidate_timings(candidate, _global_roads)
    result = fixed_main(candidate_timings=candidate_timings, headless=True,
                        rows=GRID_ROWS, cols=GRID_COLS, sim_duration=600,
                        random_seed=random.randint(1, 100000))
